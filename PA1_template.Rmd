...
plot(daily_num_of_steps$date, daily_num_of_steps$steps, 
     type = "h", 
...
```{r, echo = TRUE}
mean(daily_num_of_steps$steps)
median(daily_num_of_steps$steps)
```
...
sum(is.na(daily_mean$daily_mean_steps))
```
There are 8 days with NA mean values, indicating that no steps were recorded on those days.
Daily mean steps is not a good choice to replace the missing values.
However, there is no row with NA for each interval.
...
plot(new_daily_steps$date, new_daily_steps$steps, 
     type = "h", 
     main = "Total Number of Daily Steps (New)", 
     xlab = "Date", 
     ylab = "Number of steps", 
     col = "blue", 
     lwd = 8)
